...
const OrdersDashboard = () => {
    ...
    const cafeOrders = [
        { id: 101, table: 'Table 5', customer: 'Abebe Bikila', items: ['Doro Wat', 'Injera'], specialRequest: 'Extra spicy', status: 'Pending' },
        { id: 102, table: 'Table 2', customer: 'Tirunesh Dibaba', items: ['Shiro', 'Tej'], specialRequest: 'No gluten', status: 'Served' },
        { id: 103, table: 'Table 8', customer: 'Haile Gebrselassie', items: ['Kitfo', 'Kocho'], specialRequest: 'Medium rare', status: 'Pending' },
    ];

    const deliveryOrders = [
        { id: 201, customer: 'Kenenisa Bekele', location: 'Bole, near Airport', phone: '0911234567', items: ['Special Tibs', 'Veggie Combo'], paymentScreenshot: 'https://images.unsplash.com/photo-1556742049-0cfed4f7a07d?w=150&h=150&fit=crop', status: 'Out for Delivery' },
        { id: 202, customer: 'Derartu Tulu', location: 'Kazanchis', phone: '0911234568', items: ['Mahberawi'], paymentScreenshot: 'https://images.unsplash.com/photo-1556742049-0cfed4f7a07d?w=150&h=150&fit=crop', status: 'Pending' },
    ];
                ...
                {activeTab === 'cafe' ? (
                    ...
                    >
                        {cafeOrders.map((order) => (
                            <div key={order.id} className="bg-[#1A0F08] border border-white/5 rounded-3xl p-8 hover:border-primary-gold hover:shadow-[0_0_40px_rgba(212,175,55,0.15)] hover:scale-[1.02] transition-all duration-500 group relative overflow-hidden backdrop-blur-xl">
                                ...
                                <div className="flex gap-4 pt-6 border-t border-white/5 relative z-10">
                                    <button className="flex-1 py-3 bg-primary-gold text-primary-dark hover:bg-yellow-400 hover:scale-105 rounded-xl text-xs font-bold transition-all uppercase tracking-widest shadow-[0_0_20px_rgba(212,175,55,0.3)] hover:shadow-[0_0_30px_rgba(212,175,55,0.5)]">
                                        Complete
                                    </button>
                                    <button className="flex-1 py-3 bg-transparent border border-white/10 text-gray-400 hover:text-white hover:border-white/30 hover:bg-white/5 rounded-xl text-xs font-bold transition-all uppercase tracking-widest">
                                        Cancel
                                    </button>
                                </div>
                            </div>
                        ...
                        {deliveryOrders.map((order) => (
                            <div key={order.id} className="bg-[#1A0F08] border border-white/5 rounded-3xl p-8 hover:border-primary-gold hover:shadow-[0_0_40px_rgba(212,175,55,0.15)] hover:scale-[1.02] transition-all duration-500 group relative overflow-hidden backdrop-blur-xl">
                                ...
                                <div className="bg-black/40 p-4 rounded-2xl border border-white/5 mb-6 relative z-10 group-hover:border-primary-gold/20 transition-colors">
                                    ...
                                    <p className="text-white text-sm font-medium leading-relaxed">{order.items.join(', ')}</p>
                                </div>
                                ...
                                <div className="flex gap-4 pt-2 relative z-10">
                                    ...
                                    <button
                                        onClick={() => setSelectedReceipt(order.paymentScreenshot)}
